            ...
            <div className="card  add mb-lg-2 mb-1 overflow-hidden "
                ...
                {customize ?
                    <div
                        ...
                        {element.options.map((op, index) => {
                            const cols = 10 / element.options.length
                            return <div className={`col-${cols} ${index === 0 ? 'bg-primary text-white' : 'text-dark' } text-center cursor-pointer`} style={{height: 130}} onClick={() => {
                                setDoubled(false)
                                setCustomize(false)
                            }}>
                        ...
                        <div className="col-md-10 col-10 " onClick={() => {
                            onItemClick(element)
                            setValue(!value)
                            if (selectedProVeg.length === {limit}) {
                            ...
                            <div className="row g-0">
                                <div className="col-lg-4  col-md-4 col-5">
                                    {selectedProVeg.map(item => {
                                        const lmt = selectedProVeg.length <= 1 || element.fillingLimit <= 1 ? '1' : `1/${element.fillingLimit}`
                                        if (lmt === '1') {
                                            return (item.id === itemId ? <CheckSign key={item.id}/> : "")
                                        } else {
                                            return (item.id === itemId ? <ExtraQty value={lmt} key={item.id}/> : "")
                                        }
